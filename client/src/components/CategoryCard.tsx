          ...
          {hasParentCategory(category) && (
            <div className="absolute top-3 left-3">
              <Link href={`/products?category=${category.parentCategory.slug}`}>
                <span className="bg-[#8C7354]/80 text-white text-xs px-2 py-1 rounded hover:bg-[#8C7354]">
            ...
            <div className="space-x-2">
              {category.subcategories.map(sub => (
                <Link key={sub.id} href={`/products?category=${sub.slug}`}>
                  <span className="inline-block text-xs text-[#8C7354] bg-[#F9F5E7] px-2 py-1 rounded-full hover:bg-[#A38F71]/10">
                    {sub.name}
                  </span>
                </Link>
              ))}
